...
1. DOCX kodifikasi terkoreksi dan `SISTEM_PAKAR_FIQIH_HAID_MASTER.html` adalah sumber kanonik bersama.
2. Konflik panduan Mubtada'ah Mumayyizah diselesaikan eksplisit oleh pengguna pada 22 Agustus 2026: aturan DOCX berlaku, yaitu daur pertama menunggu darah tembus 15 hari sebelum mandi dan qadha shalat masa darah lemah.
...
5. HTML kanonik dan `POHON_KEPUTUSAN_LOGIKA_SISTEM.md` diselaraskan dengan keputusan tersebut; DOCX tidak memerlukan perubahan karena sudah memuat aturan yang dipilih.
...
| F-06 | Persistensi hanya menerima skema v1 dan membuang versi lain. | Penambahan field akan memutus riwayat lama. | Diperbaiki dengan migrasi bertahap hingga v4 |
...
| F-09 | Sifat darah selalu dianggap dapat diamati. | Nilai bawaan atau data parsial dapat memicu tamyiz seolah-olah merupakan observasi lengkap. | Diperbaiki dengan gerbang pengetahuan sifat darah |
...
| Tamyiz hanya dinilai bila pengguna mampu mengamati dan semua sifat darah alami diketahui | gerbang observasi | `knowsBloodCharacteristics` + nilai `TIDAK_DIKETAHUI`; data tersembunyi tetap disimpan tetapi diabaikan | kelompok “kontrol pengetahuan sifat darah” | Otomatis |
| Mubtada'ah mumayyizah | `MUST-01`/`TAMYIZ-VALID` | Tamyiz sah; daur pertama mandi pada masuk hari ke-16 dan qadha shalat masa darah lemah, daur berikutnya mandi saat transisi kuat→lemah | kelompok tujuh golongan; tes penundaan mandi daur pertama | Otomatis |
...
| Mu'tadah mumayyizah: tamyiz mengalahkan adat | `MUST-03`/`TAMYIZ-VALID` | Cabang tamyiz dijalankan sebelum kembali ke adat | “mendahulukan tamyiz Mu'tadah…” | Otomatis |
...
| Darah nifas >60 hari kembali ke adat nifas, tamyiz bila observasi lengkap, atau minimum resolusi aplikasi | `NIFAS-OVER-60` | Urutan adat → tamyiz bergated → satu menit | tes nifas tamyiz aktif/nonaktif | Otomatis |
...
| Riwayat v1–v3 harus tetap terbaca | `HISTORY_SCHEMA_VERSION=4` | Migrasi lama menetapkan `knowsBloodCharacteristics=true` agar hasil historis tetap sama; v4 mempertahankan nilainya | tes migrasi v1/v2/v3 dan v4 | Otomatis |
| Ekspor memuat semua input baru | ekspor | JSON envelope v4 tetap lossless; TXT menampilkan status pengetahuan dan menyembunyikan sifat saat tidak aktif | tes ekspor + inspeksi kode | Otomatis |
...
- Penilaian aroma, warna, dan kekentalan bergantung observasi pengguna. Checkbox hanya menyatakan ketersediaan observasi; kategori Mumayyizah/Ghairu Mumayyizah tetap ditentukan engine. Asal darah tetap independen dan selalu dapat diisi.
...
- `npm test`: 51 tes lulus pada finalisasi kontrol pengetahuan sifat darah.
- `npm run lint`: lulus tanpa warning.
- `npm run build`: lulus (`tsc -b && vite build`).
- Smoke test browser memastikan toggle postpartum dan pengetahuan sifat darah default mati; karakteristik dapat ditampilkan, disembunyikan, dan dipulihkan tanpa kehilangan nilai; tidak ada error console. Field lokasi tetap dinonaktifkan melalui feature flag.
...
- `src/lib/history.ts` — skema v4, migrasi, JSON/TXT.
